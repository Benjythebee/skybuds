...
import { NameStorage, SkyBuds, SkyBudsMetadata } from "../typechain-types";
...
describe("SkyBuds NFT Contract", function () {
  ...
  let nameStorageAddress: string;
  let nameStorageMetadata: NameStorage
  ...
  before(async function () {
    ...
    // Deploy the contract
    const NameStorageFactory = await ethers.getContractFactory("NameStorage");
    nameStorageMetadata = await NameStorageFactory.deploy();
    nameStorageAddress = await nameStorageMetadata.getAddress()
  })

  beforeEach(async function () {
    // Get signers
    [owner, collector1, collector2] = await ethers.getSigners();
    // Deploy the NameStorage contract
    // const NameStorageFactory = await ethers.getContractFactory("NameStorage");
    // nameStorageMetadata = await NameStorageFactory.deploy();
    // const nameStorageAddress = await nameStorageMetadata.getAddress()
    // Deploy the contract
    const SkyBudsMetadataFactory = await ethers.getContractFactory("SkyBudsMetadata");
    skyBudsMetadata = await SkyBudsMetadataFactory.deploy(nameStorageAddress);
    const skyBudsMetaAddress = await skyBudsMetadata.getAddress()

    await setNamesForContract(nameStorageMetadata)
  ...
  describe("Deployment", function () {
    ...
    it("name storage should have correct index", async function () {
    const name = await nameStorageMetadata.getNameForIndex(1)
    const name2 = await nameStorageMetadata.getNameForIndex(2)
    const lastname = await nameStorageMetadata.getNameForIndex(names.length)

    expect(name).to.equal('a');
    expect(name2).to.equal('b');
    expect(lastname).to.equal(names[names.length-1]);
    })
...
skyBuds.updateWearables(2,[1,2,3])
  ...
  describe("Token URI", function () {
    ...
    it("Should get tokenURI of minted token", async function () {
      ...
      expect(!!parsed.description).to.be.true;
  ...
  describe("Limit 1k", function () {

    it("Minting 1001 items should fail", async function () {
        for(const i of Array(1000).keys()) {
          await skyBuds.mint([],20,50,1,'#ffffff',"/9j/4AAQSkZJRgABAQAAAQABAAD/4gHYSUNDX1BST0ZJTEUAAQEAAA");
        }

        await expect(
          skyBuds.mint([],20,50,1,'#ffffff',"/9j/4AAQSkZJRgABAQAAAQABAAD/4gHYSUNDX1BST0ZJTEUAAQEAAA")
