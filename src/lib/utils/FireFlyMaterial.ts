...
export class FireFlyMaterial extends ShaderMaterial {
	constructor(options: FireFlyMaterialOptions = {}) {
		...
		super({
            ...
            uniform vec3 uColor;
			uniform float opacity; // New uniform for controlling overall transparency
            varying float vOffset;
            ...
            void main() {
                ...
                float alpha = clamp((glow + disk) * flash * opacity, 0.0, 1.0); // Apply opacity uniform to alpha
	...
	setOpacity(value: number) {
		this.uniforms.opacity.value = value; // Set the opacity uniform
	}
