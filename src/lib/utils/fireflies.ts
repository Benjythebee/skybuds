...
type Props = {
	...
	groupRadius: Vector3; // Radius of each group
...
export class FireFlies {
	...
	private groupRadius: Vector3; // Radius for grouping fireflies
	...
	constructor(_scene: Scene, props: Props = defaultProps) {
		...
		const fireflyGeometry = new PlaneGeometry(0.05, 0.05); // Adjust the size of the firefly as desired
		...
		for (let i = 0; i < groupCount; i++) {
			...
			for (let j = 0; j < firefliesPerGroup; j++) {
				...
				const offset = new Vector3(
					this.randomGaussian() * this.groupRadius.x,
					this.randomGaussian() * this.groupRadius.y,
					this.randomGaussian() * this.groupRadius.z
				);
